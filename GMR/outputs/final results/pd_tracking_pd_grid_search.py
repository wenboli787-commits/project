...
out_root = repo / 'outputs/final results/pd tracking pd grid search'
...
for kp in kp_values:
    for kd in kd_values:
        ...
        min_root_height = min((as_float(r.get('root_height_min')) for r in motion_rows), default=float('nan'))
        row = {
            'kp': kp,
            'kd': kd,
            'torque_clip': 250,
...
for r in ranked[:10]:
    print(
        ...
        f"mean_joint_err={r['mean_joint_position_error']:.4f} min_root={r['min_root_height']:.4f}"
    )
print(f"[grid] summary: {out_root / 'pd_grid_summary.csv'}")
print(f"[grid] best: {out_root / 'best_pd_params.json'}")